...
async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!headers.has("X-Language"))
    headers.set("X-Language", preferredLanguage());
...
export async function uploadDocument(
  file: File,
): Promise<{ document_id: string; name: string }> {
  const fd = new FormData();
  fd.append("file", file);
  return apiFetch<{ document_id: string; name: string }>(
    `/api/v1/docs/upload`,
    {
      method: "POST",
      body: fd,
...
export async function chatStream(
  ...
  const headers: Record<string, string> = {
    ...
    "X-Language": options?.language || preferredLanguage(),
  };
...
export async function fetchConversations(): Promise<Conversation[]> {
  const res = await apiFetch<{ items: Conversation[] }>(
...
export async function createServerConversation(
  title?: string,
): Promise<{ id: string; title: string; series?: number }> {
  return apiFetch<{ id: string; title: string; series?: number }>(
    `/api/v1/chat/conversations`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function renameServerConversation(
  id: string,
  title: string,
): Promise<{ ok: boolean }> {
...
export async function deleteServerConversation(
  ...
  if (typeof series === "number") {
    ...
    return apiFetch<{ ok: boolean }>(`/api/v1/chat/conversations/${id}?${q}`, {
      method: "DELETE",
    });
  }
  return apiFetch<{ ok: boolean }>(`/api/v1/chat/conversations/${id}`, {
...
export async function adminSetUserAuth(
  userId: string,
  auth: "admin" | "manager" | "user" | "administrator",
): Promise<{ ok: boolean }> {
...
export async function adminGetUserPermissions(
  ...
  return apiFetch<{ document_ids: string[] }>(
...
export async function getPrompts(): Promise<PromptSection[]> {
  const res = await apiFetch<{ sections: PromptSection[] }>(`/api/v1/prompts`);
  return res.sections || [];
}
...
export async function updatePrompts(
  ...
  return apiFetch<{ ok: boolean }>(`/api/v1/prompts`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function resetPrompt(key: string): Promise<{ ok: boolean }> {
  return apiFetch<{ ok: boolean }>(`/api/v1/prompts/${key}`, {
    method: "DELETE",
  });
}

export async function resetAllPrompts(): Promise<{ ok: boolean }> {
  return apiFetch<{ ok: boolean }>(`/api/v1/prompts/reset`, { method: "POST" });
}
